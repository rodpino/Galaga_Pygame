    ...
    def shoot(self, dt):
        ...
        if current_time - self.last_shot_time > self.shoot_cooldown:
            ...
            if random.random() < 0.1:  # Probabilidad de disparo
                ...
                laser = AlienLaser(self.rect.center, player_position)
...
class AlienAzul(Aliens):
    ...
    def execute_phases(self):
        if self.game.current_group == 2 and self.group_id == 2:
...
class AlienRojo(Aliens):
    ...
    def control_points_8(self):
        return np.array([WIDTH - 56, 517]), np.array([WIDTH - 123, 619]), np.array([WIDTH - 248, 561]), np.array([212, 463])
    ...
    def update(self, dt):
        ...
        self.image = self.original_image.copy()
        self.mask = pygame.mask.from_surface(self.image)  # Actualizar la máscara con la nueva imagen
        if self.game.current_group == 1 and self.group_id == 1:
            self.execute_phases_group_1()
        elif self.game.current_group == 2 and self.group_id == 2:
            self.execute_phases_group_2()
