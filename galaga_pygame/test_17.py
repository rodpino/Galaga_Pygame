...
class Game:
    def __init__(self):
        ...
        SPRITE_SHEET_PATH = os.path.join(PROJECT_ROOT, "asset", "Galaga_SpritesSheet.png")
        SOUND_PATH = os.path.join(PROJECT_ROOT, "asset", "sound_shoot_3.wav")
        ...
        try:
            ...
            self.FONT_score = pygame.font.SysFont(None, 20)

        try:
            self.SPRITE_SHEET = pygame.image.load(SPRITE_SHEET_PATH).convert_alpha()
        except Exception as e:
            print(f"[ERROR] No se pudo cargar la hoja de sprites: {SPRITE_SHEET_PATH}\n{e}")
            self.SPRITE_SHEET = pygame.Surface((16, 16))  # Placeholder para evitar crash

        try:
            self.sound_shoot = pygame.mixer.Sound(SOUND_PATH)
        except Exception as e:
    ...
    def check_for_collision(self):
        ...
        # Verificar si la formación de alienígenas está vacía               
...
class Alien(pygame.sprite.Sprite):
    ...
    def __init__(self, game, mIndex, formation, alien_type, bezier_id):
        ...
        self.capture_laser = CaptureLaser(self)
         # Agregar el jugador de captura al grupo
        ...
        self.precalculate_bezier_curves()
    def start_capture(self):
        self.capture_laser.define_capture_curves_1()  # Llama a la captura específica
        self.capture_laser.define_capture_curves_2()  # Llama a la captura específica
            ...
            if current_time - self.last_shot_time > self.shoot_cooldown:
                ...
                if random.random() < 0.1:  # Probabilidad de disparo
                    ...
                    if random.random() < 0.5:  # 50% de disparos precisos
                        laser = AlienLaser(self.game, self.rect.center, player_position, error_margin=0)
                    else:  # 50% de disparos con variación
                        laser = AlienLaser(self.game, self.rect.center, player_position, error_margin=75)
    ...
    def check_capture_collision(self):
        # Solo chequea si hay animación de captura activa
        if self.pausing and self.capture_animation_frame:
            player = self.formation.game.player_sprite
            player_mask = player.mask
            player_rect = player.rect
            animation_frame = self.capture_animation_frame
            # Calcula dónde estaría el rayo en pantalla (usa la misma lógica que draw)
            rect = self.image.get_rect(center=(int(self.x), int(self.y)))
            frame_rect = animation_frame.get_rect(midtop=rect.midbottom)
            ray_mask = pygame.mask.from_surface(animation_frame)
            offset = (player_rect.left - frame_rect.left, player_rect.top - frame_rect.top)
            if ray_mask.overlap(player_mask, offset) and not getattr(player, 'captured', False):
                player.captured = True
                print("¡Colisión: player capturado por rayo!")

            if ray_mask.overlap(player_mask, offset) and not getattr(player, 'captured', False):
                player.captured = True
                player.capturing_boss = self  # Referencia al boss
                # Calcula el offset del jugador respecto al boss en ese momento
                player.offset_x = player.rect.centerx - self.rect.centerx
                player.offset_y = player.rect.centery - self.rect.centery
                self.returning_with_captured_player = True
                print("¡Colisión: player capturado por rayo!")
    ...
    def update(self, delta_time):
        ...
        for alien in self.formation.aliens:
            if hasattr(alien, 'check_capture_collision'):
                alien.check_capture_collision()
    
        if getattr(self, 'returning_with_captured_player', False):
            # Mover el boss hacia el grid (usa tu lógica de interpolación de regreso)
            self.final_move_progress += self.speed * delta_time
            if self.final_move_progress >= 1.0:
                self.final_move_progress = 1.0
                self.returning_with_captured_player = False
                # Aquí puedes dejar al player como capturado o liberarlo
                # Por ejemplo:
                player = self.formation.game.player_sprite
                player.captured = False
                player.capturing_boss = None
                # Puedes “liberar” al jugador aquí, o dejarlo como “secuestrado” para rescate

            self.x = (1 - self.final_move_progress) * self.start_x + self.final_move_progress * self.initial_target_x
            self.y = (1 - self.final_move_progress) * self.start_y + self.final_move_progress * self.initial_target_y

            # También mueve al jugador junto al boss
            player = self.formation.game.player_sprite
            player.rect.centerx = self.x + player.offset_x
            player.rect.centery = self.y + player.offset_y



    def precalculate_bezier_curves(self, num_points=20):
        self.curve_draw_cache = []
        # Curvas normales (movimiento)
        if hasattr(self, 'curves') and self.curves:
            for curve in self.curves:
                points = [self.bezier_curve(curve, i / num_points) for i in range(num_points + 1)]
                self.curve_draw_cache.append(points)
        else:
            self.curve_draw_cache = []

        # Curvas de ataque (opcional)
        self.attack_curve_draw_cache = []
        if hasattr(self, 'attack_curves') and self.attack_curves:
            for curve in self.attack_curves:
                points = [self.bezier_curve(curve, i / num_points) for i in range(num_points + 1)]
                self.attack_curve_draw_cache.append(points)
        else:
            self.attack_curve_draw_cache = []


    def draw_bezier_path(self, surface):
        # Activa esto para debug: self.debug_draw_path = True
        # if not getattr(self, 'debug_draw_path', True):
        #     return
        
        curve_colors = [(255, 255, 255), (0, 255, 0), (0, 0, 255), (255, 0, 0)]

        # Dibuja curvas normales precalculadas
        # if hasattr(self, 'curve_draw_cache') and self.curve_draw_cache:
        #     for idx, points in enumerate(self.curve_draw_cache):
        #         color = curve_colors[idx % len(curve_colors)]
        #         pygame.draw.lines(surface, color, False, [(int(x), int(y)) for x, y in points], 1)
        
        # Dibuja curvas de ataque precalculadas (si existen y si quieres mostrarlas)
        if hasattr(self, 'attack_curve_draw_cache') and self.attack_curve_draw_cache:
            for points in self.attack_curve_draw_cache:
                pygame.draw.lines(surface, (255, 255, 0), False, [(int(x), int(y)) for x, y in points], 2)
    ...
    def draw(self, surface):
        if self.active or self.arrived or self.attack_mode or self.pausing:
            ...
            if getattr(self, 'captured', False) and self.capturing_boss:
                # Seguir al boss
                self.rect.centerx = self.capturing_boss.x + self.offset_x
                self.rect.centery = self.capturing_boss.y + self.offset_y     
                print("Player capturado por el boss, siguiendo al boss en su movimiento.")
...
class Formation:
    ...
    def add_aliens_to_group(self, alien_type, count, offset=0):
        ...
        for mIndex in range(count):
            ...
            alien = Alien(self.game, mIndex + offset, self, alien_type, bezier_id)
    ...
    def capture_formation(self, formation):
        ...
        # Chequear si ya hay un boss en modo captura
        any_boss_capturing = any(
            alien.attack_mode and alien.alien_type in ['boss_green', 'boss_blue']
            for alien in self.aliens
        )
        if any_boss_capturing:
            return  # No permitir más de un boss capturando a la vez
        ...
        for alien in attack_group:
            ...
            if alien.alien_type == 'boss_green' or alien.alien_type == 'boss_blue':
                ...
                if alien.mIndex % 2 == 0:
                    alien.capture_laser.define_capture_curves_1()
                else:
                    alien.capture_laser.define_capture_curves_2()
